...
const ProjectCard = ({ item }) => {
  ...
  return (
    <div
      key={item._id}
      className=" flex flex-col items-center p-2 rounded-md border border-primary w-full max-w-[1200px] my-1 "
    >
      <div className="flex flex-col items-center w-full px-2 ">
        <Slideshow photos={photos} />
      </div>
      <div className="flex flex-col items-start p-2">
        <h1 className="font-bold text-primary underline underline-offset-3 decoration-accent text-xl md:text-2xl my-2">
          {item.title}
        </h1>
