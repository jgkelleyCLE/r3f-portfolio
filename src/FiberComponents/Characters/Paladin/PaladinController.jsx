...
const PaladinController = ({ isJumping, setIsJumping, movement }) => {
  const rb = useRef();
  ...
  const respawn = () => {
    ...
    const rotation = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, THREE.MathUtils.degToRad(180), 0));
  ...
  const goToProjects = () => {
    ...
    const rotation = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, THREE.MathUtils.degToRad(90), 0));
    rb.current.setRotation(rotation);
  };
