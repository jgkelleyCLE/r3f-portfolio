...
const Timer = () => {
  ...
  return (
    <div
      className={`bg-black/50 rounded-md font-mono text-lg md:text-3xl ${
        lowGravity ? 'text-red-500' : 'text-lime-400'
      } px-2 flex flex-col items-center`}
    >
      <h1 className={`${lowGravity ? 'line-through' : null}`}>
        {formatTime(elapsedTime)}
      </h1>
      <h1 className={`text-[9px] md:text-xs -mt-1 md:-mt-1`}>
        {lowGravity ? 'Low Gravity' : 'Challenge Timer'}
      </h1>
    </div>
  );
};
